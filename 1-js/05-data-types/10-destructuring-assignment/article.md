...
Важно понимать, что значение по умолчанию применяется только для undefined. Если в массиве окажется null, оно будет использовано как есть:

```js run
// значения по умолчанию и null
const [firstName = 'Julius', surname = 'Caesar'] = [null, undefined];

alert(firstName); // null, т.к. null не заменяется значением по умолчанию
alert(surname); // Caesar, значение по умолчанию используется для undefined
```

Эта особенность важна при работе с массивами, в которых могут встречаться null значения.
